...
function upsertArticleStub(row: SingleArticleRow) {
  return putArticle(`${row.fakeid}:${row.aid}`, buildVirtualArticle(row));
}
...
async function updateRowFromHtml(row: SingleArticleRow) {
  ...
  await putArticle(`${row.fakeid}:${row.aid}`, {
...
async function deleteRowData(row: SingleArticleRow) {
  ...
  await Promise.all([
    deleteArticle(key),
    $fetch('/api/store/html/delete', { method: 'POST', body: { url: row.link } }),
  ]);
}
