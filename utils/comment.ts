...
/**
 * 渲染评论为 HTML
 * @param electedComments 精选评论列表
 * @param commentNum 评论总数
 * @param replyMap 评论回复映射 { content_id: reply_list }
 */
export function renderCommentsHtml(
  electedComments: any[],
  commentNum?: number,
  replyMap?: Map<string, any[]>,
): string {
  if (!electedComments || electedComments.length === 0) {
    return ''
  }

  let commentHTML = ''
  commentHTML += '<div style="max-width: 667px;margin: 0 auto;padding: 10px 10px 80px;">'
  commentHTML += `<p style="font-size: 15px;color: #949494;">留言 ${commentNum || electedComments.length}</p>`
  commentHTML += '<div style="margin-top: -10px;">'

  for (const comment of electedComments) {
    ...
    // 评论回复
    let replyList: any[] = []
    if (replyMap && replyMap.has(comment.content_id)) {
      replyList = replyMap.get(comment.content_id)!
    } else if (comment.reply_new && comment.reply_new.reply_list && comment.reply_new.reply_list.length > 0) {
      replyList = comment.reply_new.reply_list
    }
